...
select(sf_gov, country, rescaled_sf, capped_mv_wts, capped_wts) %>%
  mutate(rescaled_sf = scales::percent(rescaled_sf, 0.01), 
         capped_mv_wts = scales::percent(capped_mv_wts, 0.01),
         capped_wts = scales::percent(capped_wts, 0.01)) %>%
  knitr::kable()
```
...
select(sf_ilb, country, rescaled_sf, capped_mv_wts, capped_wts)  %>%
  mutate(rescaled_sf = scales::percent(rescaled_sf, 0.01), 
         capped_mv_wts = scales::percent(capped_mv_wts, 0.01),
         capped_wts = scales::percent(capped_wts, 0.01)) %>%
  knitr::kable()
...
# Input
gov_disp <- select(sf_gov, country, rescaled_sf, capped_mv_wts, capped_wts) 
ilb_disp <- select(sf_ilb, country, rescaled_sf, capped_mv_wts, capped_wts)
...
addDataFrame(gov_disp, sheet, startRow = 6, startColumn = 1, 
             colnamesStyle = TABLE_COLNAMES_STYLE,
             rownamesStyle = TABLE_ROWNAMES_STYLE, row.names = FALSE,
             colStyle = list('2' = PERCENT_STYLE, '3' = PERCENT_STYLE, '4' = PERCENT_STYLE))

addDataFrame(ilb_disp, sheet, startRow = 6, startColumn = 7, 
             colnamesStyle = TABLE_COLNAMES_STYLE,
             rownamesStyle = TABLE_ROWNAMES_STYLE, row.names = FALSE,
             colStyle = list('2' = PERCENT_STYLE, '3' = PERCENT_STYLE, '4' = PERCENT_STYLE))
