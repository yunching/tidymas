...
```{r build_portfolios_and_get_returns}
...
active_risk_prev <- calc_active_risk(unwt_returns$sim, 
                                prev_strat,
...
active_risk_curr <- apply(scenarios, 1, 
                     function(x) {
                       calc_active_risk(unwt_returns$sim, 
                                        curr_strat,
...
scenario_return <- apply(scenarios, 1, 
                           function(x) {
                             simulate_history(unwt_returns$sim, 
                                      curr_strat, 
...
ret_by_owner <- wt_returns$actual %>%
  custom_grouping(instr_df, "owner")

actual_corr <- calc_cor(ret_by_owner,
                        start_date = curr_date - months(3),
                        end_date = curr_date)

exp_return <- simulate_history(unwt_returns$sim, 
                               curr_strat, 
                               curr_date - months(3), curr_date)
exp_ret_by_owner <- exp_return %>%
  custom_grouping(instr_df, "owner")

exp_corr <- calc_cor(exp_ret_by_owner,
                        start_date = curr_date - months(3),
                        end_date = curr_date)
```
...
risk_budget <- 80
current_quarter <- month(curr_date) %/% 3 + 1
prev_quarter <- if (month(curr_date) %/% 3 == 0) 4 else month(curr_date) %/% 3
...
**The RMD ex BW active risk utilization for Q`r current_quarter` is expected to be around `r sprintf("%.1f", current_abs_active_risk)`bps in our baseline scenario**, `r if (current_abs_active_risk > risk_budget) "above" else "below"` the available risk budget for FY18/19 and `r if (current_abs_active_risk > prev_active_risk) "above" else "below"` our estimates for the last quarters (`r prev_quarter`Q: `r sprintf("%.1f",prev_abs_active_risk)`bps). Assuming an information ratio of `r sprintf("%.1f", info_ratio)`, the expected portfolio excess return will be `r sprintf("%.1f", info_ratio * current_abs_active_risk)` (annualized). Under a stress scenario similar to the GFC and the Taper Tantrum, we expect absolute active risk utilization to increase to around `r sprintf("%.1f", gfc_abs_active_risk)` and `r sprintf("%.1f", taper_abs_active_risk)`bps respectively
...
2. The most diversifying strategy in the portfolio is the `r most_diverse$name` from the `r most_diverse$owner`, with a `r sprintf("%.1f", most_diverse$risk)`% risk contribution
...
strat_percent_size <- get_strat_size(strat_headline_size$actual, curr_date) %>%
  gather(strategy, percent_size) %>%
  mutate(size_percent = percent_size)
...
end_date_returns <- curr_date
start_date_returns <- end_date_returns - months(12)


# Extract return data based on start_date
actual_ret_filtered <- wt_returns$actual %>% 
  filter(date >= start_date_returns & date <= end_date_returns) %>%
...
uniq_unwt_ret <- unwt_returns$sim[,c("date", names(curr_strat))] %>%
...
unwt_ret_roll <- apply(remove_date(unwt_returns$sim), 2, RcppRoll::roll_sum, n = window) %>% as.data.frame
unwt_ret_roll <- cbind(data.frame(date = unwt_returns$sim$date[-(1:(window-1))]), unwt_ret_roll)

sim_ret_roll <- apply(remove_date(wt_returns$sim), 2, RcppRoll::roll_sum, n = window) %>% as.data.frame
sim_ret_roll <- cbind(data.frame(date = wt_returns$sim$date[-(1:(window-1))]), sim_ret_roll)

######################################
# Plot heatmap of returns across time
######################################

active_unwt_ret_roll <- unwt_ret_roll[,c("date", names(curr_strat))]
