...
unwt_ret_sp <- spread(unwt_return, strategy, return)
wt_ret_sp <- spread(wt_return$sim, strategy, wt_return)

unwt_ret_roll <- apply(remove_date(unwt_ret_sp), 2, RcppRoll::roll_sum, n = window) %>% as.data.frame
unwt_ret_roll <- cbind(data.frame(date = unwt_ret_sp$date[-(1:(window-1))]), unwt_ret_roll)

sim_ret_roll <- apply(remove_date(wt_ret_sp), 2, RcppRoll::roll_sum, n = window) %>% as.data.frame
