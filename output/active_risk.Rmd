...
scenarios <- rbind(scenarios_input, 
      data.frame(period = c("Last3M"), start_date = c(today() - months(3)), end_date = c(today()))) 
...
curr_strat <- get_strat_size(strat_headline_size$actual, as.Date("2018-11-28"))
...
historical_return <- apply(scenarios, 1, 
                           function(x) {
...
wt_returns_w_group <- wt_returns$actual %>%
  gather(strategy, returns, -date) %>%
  left_join(select(instr_df, strategy, owner, type), by = "strategy")

actual_team_ret <- wt_returns_w_group %>%
  group_by(date, owner) %>%
...
actual_corr <- calc_cor(actual_team_ret,
                        start_date = as.Date("2017-10-31"),
