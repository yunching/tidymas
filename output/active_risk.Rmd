...
```{r build_portfolios_and_get_return}
...
unwt_return <- map2(wt_return, strat_headline_size, calc_strat_unwt_return)
```
...
active_risk_prev <- calc_active_risk(unwt_return$sim, 
                                prev_strat,
...
active_risk_curr <- apply(scenarios, 1, 
                     function(x) {
                       calc_active_risk(unwt_return$sim, 
                                        curr_strat,
...
scenario_return <- apply(scenarios, 1, 
                           function(x) {
                             simulate_history(unwt_return$sim, 
                                      curr_strat, 
...
exp_return <- simulate_history(unwt_return$sim, 
                               curr_strat, 
...
uniq_unwt_ret <- unwt_return$sim[,c("date", names(curr_strat))] %>%
...
unwt_ret_roll <- apply(remove_date(unwt_return$sim), 2, RcppRoll::roll_sum, n = window) %>% as.data.frame
unwt_ret_roll <- cbind(data.frame(date = unwt_return$sim$date[-(1:(window-1))]), unwt_ret_roll)

sim_ret_roll <- apply(remove_date(wt_return$sim), 2, RcppRoll::roll_sum, n = window) %>% as.data.frame
sim_ret_roll <- cbind(data.frame(date = wt_return$sim$date[-(1:(window-1))]), sim_ret_roll)
