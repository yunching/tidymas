...
#   na.omit ->
#   bbg_data
# load_bbg_data(dbname = "../../mydatabase.db")

load_db(dbname = "../../mydatabase.db") %>% 
...
asset. = bbg_data2 %>% select(-Volume) %>% nest(data = c(date, PX_OPEN, PX_HIGH, PX_LOW, PX_LAST))
...
table1 <- function(tibbledata){
  kable(tail(tibbledata[,c(1:5, 13:18)],21), col.names=colnames1, align=rep('c', 11))}
table2 <- function(tibbledata) {
  kable(tail(tibbledata[,c(1, 6:12, 19:23)],21), col.names=colnames2, align=rep('c', 13))}
cat("RX1")
table1(RX1.df)
table2(RX1.df)
